...
We created an algorithm that forecasts stock price changes from news article content and historic stock prices.
...
negative increase in the mentioned stock. \\  \\
We propose a model that will predict the stock change as a percentage by
applying a probabilistic model to the neighbors of a given document that
will be used to weight each percentage to finally calculate the
aggregate percentage. Two variations of this model will be tested:
\begin{itemize}
...
set of synonyms generated from our stock stockers from
...
\begin{itemize}
  ...
  Again, the test group was then randomly shuffled and place into 5 folds.
\end{itemize}
